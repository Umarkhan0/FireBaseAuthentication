      ...
      name.trim() ?
        createUserWithEmailAndPassword(auth, email, password)
          .then((userCredential) => {
            ...
            document.getElementById("LogSingMain").remove()
            document.getElementById("containerHello").innerHTML = `
            <img class="helloImg" src="images/472108440_HELLO_STICKER_400px.gif" alt="Hello">
                    <p class="emailText">${email}</p>
                      <button class="verifyBtn" id="VerifyFunDir">Verify</button>
                   <p id="CheckVerSing class="trueFalse">Verification: <span class="italic">${auth.currentUser.emailVerified}</span></p>
         <button id="signOut" class="signOut">Sign out </button>
            ...
            let VerifyFunDir = document.getElementById("VerifyFunDir");
            VerifyFunDir.addEventListener("click", () => {
            ...
            auth.currentUser.emailVerified == true ? document.getElementById("VerifyFunDir").disabled = true
            :console.log("nahi")
          ...
          .catch((error) => {
            ...
            let errorBox = document.getElementById("errorlog");
        ...
        errorMessage == 'Firebase: Error (auth/email-already-in-use).' ? errorBox.innerHTML = `
        <p class="error">Already use Email</p> 
        `
            :
            
            

            errorMessage == 'Firebase: Error (auth/missing-password).' ? password.style.border = "2px red solid"
              :
              errorMessage == 'Firebase: Error (auth/invalid-email).' ? emailChack.style.border = "2px red solid"
                :
                errorMessage == 'Firebase: Error (auth/missing-email).' ? emailChack.style.border = "2px red solid"
                  :
                  errorMessage == 'Firebase: Password should be at least 6 characters (auth/weak-password).' ? alert("Strong password")
                    :
                    console.log("none")
  ...
  signUpBtn.addEventListener('click', () => {
    ...
    signInWithEmailAndPassword(auth, signEmail.value, signPassword.value)
      .then((userCredential) => {
        ...
        document.getElementById("containerHello").innerHTML = `
    <img class="helloImg" src="images/472108440_HELLO_STICKER_400px.gif" alt="Hello">
             <p class="emailText">${signEmail.value}</p>
              <button class="verifyBtn" id="VerifyFunDir">Verify</button>
           <p id="CheckVerSing" class="trueFalse">Verification: <span class="italic">${auth.currentUser.emailVerified}</span></p>
         <button id="signOut" class="signOut">Sign out </button>
...
const unsubscribe = auth.onAuthStateChanged((user) => {
  ...
  if (localStorage.user) {
    ...
    <img class="helloImg" src="images/472108440_HELLO_STICKER_400px.gif" alt="Hello">
         <p class="emailText"> ${getobj.signEmail}</p>
         <button class="verifyBtn" id="VerifyFun">Verify</button>
         <p id="CheckVerify" class="trueFalse">Verification: <span class="italic">${user.emailVerified}</span></p>
         <button id="signOut" class="signOut">Sign out </button>
         </div>
